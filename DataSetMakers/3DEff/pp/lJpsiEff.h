...
/*  TGraphAsymmErrors *gSingleMuW[ngraph], *gSingleMuW_LowPt[ngraph];
  ...
  for (int i=0; i<ngraph; i++) {
    ...
    "(0.7788*TMath::Erf((x-1.1903)/1.9880))/(0.7364*TMath::Erf((x-1.2538)/2.2530))");
...
//            f1DEffPtFit[nidx]->SetParLimits(2,2.35,2.45);
          }
        } else { // PbPb PRMC
          if (ymin==2.0 && ymax==2.4 && centmin==0 && centmax==4) {
...
//          f1DEffPtFit[nidx]->SetParLimits(1,-10,3);
      ...
      TFitResultPtr res = g1DEffPtFit[nidx]->Fit(Form("%s_TF",h1DEffPtFit[nidx]->GetName()),"R S EX0");
      int counter=0;
      if (0 != res->Status()) {
        while (1) {
          counter++;
          res = g1DEffPtFit[nidx]->Fit(Form("%s_TF",h1DEffPtFit[nidx]->GetName()),"R S EX0");
