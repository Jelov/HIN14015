...
void check3DHisto() {
  ...
  for (int i=0; i<nbinscent-1; i++) {
    ...
    TH3D *hNPEff = (TH3D*)NPOut->Get(Form("hEff3D_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
    className = "PRJpsi";
    TH3D *hPREff = (TH3D*)PROut->Get(Form("hEff3D_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
    
    className = "NPJpsi";
    cout << "NPMC " << Form("hEff3D_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax) << endl;
    if (hNPEff) {
...
void lJpsiEff_resolAll(bool absRapidity=true, bool logy=false, bool isPbPb=false) {
  gROOT->Macro("../JpsiStyle.C");
  gStyle->SetOptStat(1);
  ...
  string className = "NPJpsi";
  hNPEffA = (TH1D*)NPOut->Get(Form("hresol_%s_Rap%.1f-%.1f_Pt%.1f-%.1f",className.c_str(),_ymin,_ymax,_ptmin,_ptmax));
  className = "PRJpsi";
  hPREffA = (TH1D*)PROut->Get(Form("hresol_%s_Rap%.1f-%.1f_Pt%.1f-%.1f",className.c_str(),_ymin,_ymax,_ptmin,_ptmax));
  ...
  lat->DrawLatex(0.15,0.80,testStr.second.c_str());
//  leg->AddEntry(hNPEffA,Form("%s",testStr.second.c_str()),"f");
  
//  leg->Draw();
  canvNP->SaveAs(Form("./ResolAll_Rap%.1f-%.1f_Pt%.1f-%.1f.pdf",_ymin,_ymax,_ptmin,_ptmax));
  canvNP->SaveAs(Form("./ResolAll_Rap%.1f-%.1f_Pt%.1f-%.1f.png",_ymin,_ymax,_ptmin,_ptmax));
...
void lJpsiEff_resolRap(bool absRapidity=true, bool logy=false, bool isPbPb=false) {
  gROOT->Macro("../JpsiStyle.C");
  ...
  for (int i=0; i<nbinsy-1; i++) {
    ...
    string className = "NPJpsi";
    hNPEff[i] = (TH1D*)NPOut->Get(Form("hresol_%s_Rap%.1f-%.1f_Pt%.1f-%.1f",className.c_str(),ymin,ymax,ptmin,ptmax));
    className = "PRJpsi";
    hPREff[i] = (TH1D*)PROut->Get(Form("hresol_%s_Rap%.1f-%.1f_Pt%.1f-%.1f",className.c_str(),ymin,ymax,ptmin,ptmax));
    if (logy) {
  ...
  leg->Draw();
  canvNP->SaveAs(Form("./ResolRap_Rap%.1f-%.1f_Pt%.1f-%.1f.pdf",_ymin,_ymax,_ptmin,_ptmax));
  canvNP->SaveAs(Form("./ResolRap_Rap%.1f-%.1f_Pt%.1f-%.1f.png",_ymin,_ymax,_ptmin,_ptmax));
...
void lJpsiEff_resolPt(bool absRapidity=true, bool logy=false, bool isPbPb=false) {
  gROOT->Macro("../JpsiStyle.C");
  ...
  leg->Draw();
  canvNP->SaveAs(Form("./ResolPt_Rap%.1f-%.1f_Pt%.1f-%.1f.pdf",_ymin,_ymax,_ptmin,_ptmax));
  canvNP->SaveAs(Form("./ResolPt_Rap%.1f-%.1f_Pt%.1f-%.1f.png",_ymin,_ymax,_ptmin,_ptmax));
...
void lJpsiEff_diff3D(bool absRapidity=true, bool logy=false, bool isPbPb=false) {
  gROOT->Macro("../JpsiStyle.C");
...
/*    if (nbinspt < 7 && nbinspt >= 4) {
    ...
    for (int c=0; c<nbinscent-1; c++) {
      ...
      for (int b=0; b<nbinspt-1; b++) {
        ...
        string className = "NPJpsi";
        hNPEff[i] = (TH1D*)NPOut->Get(Form("hEff_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
        className = "PRJpsi";
        hPREff[i] = (TH1D*)PROut->Get(Form("hEff_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
        if (logy) {
        ...
        hNPEff[i]->GetXaxis()->SetRangeUser(-2,3);
        if (b==0 && c==0) {
          hNPEff[i]->Draw();
        } else {
          hNPEff[i]->Draw("same");
...
void lJpsiEff_diffPt(bool absRapidity=true, bool logy=false, bool isPbPb=false) {
  gROOT->Macro("../JpsiStyle.C");
  ...
  for (int i=0; i<nbinspt-1; i++) {
    ...
    hPREff[i] = (TH1D*)PROut->Get(Form("hEff_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
    if (logy) {
      SetHistStyle(hNPEff[i],i,0,1E-3,5.3);
      SetHistStyle(hPREff[i],i,1,1E-3,5.3);
    } else {
  ...
  leg->Draw();
  canvNP->SaveAs(Form("./Pt_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.pdf",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
  canvNP->SaveAs(Form("./Pt_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.png",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
...
void lJpsiEff_diffRap(bool absRapidity=true, bool logy=false, bool isPbPb=false) {
  gROOT->Macro("../JpsiStyle.C");
  ...
  canvNP->cd();
  leg->Draw();
  canvNP->SaveAs(Form("./Rap_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.pdf",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
  canvNP->SaveAs(Form("./Rap_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.png",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
...
void lJpsiEff_diffCent(bool absRapidity=true, bool logy=false, bool isPbPb=false) {
  gROOT->Macro("../JpsiStyle.C");
  ...
  for (int i=0; i<nbinscent-1; i++) {
    ...
    if (isPbPb) leg->AddEntry(hNPEff[i],Form("Cent. %.0f-%.0f%%",centmin*2.5,centmax*2.5),"pe");
    leg->AddEntry(ghost,Form("%.1f %%, %.1f %%",hPREffAvg*100,hNPEffAvg*100),"");
  ...
  canvNP->cd();
  leg->Draw();
  canvNP->SaveAs(Form("./Cent_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.pdf",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
  canvNP->SaveAs(Form("./Cent_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.png",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
...
void lJpsiEff_all(bool absRapidity=true, bool logy=false, bool isPbPb=false) {
  gROOT->Macro("../JpsiStyle.C");
  ...
  // All integrated y and pT efficiency
  double _ymin=yarray[0]; double _ymax=yarray[nbinsy-1];
  double _ptmin=ptarray[0]; double _ptmax=ptarray[nbinspt-1];
  int _centmin=centarray[0]; int _centmax=centarray[nbinscent-1];

  TH1D *hNPEffA = (TH1D*)NPOut->Get(Form("hEff1D_NPJpsi_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
  TH1D *hPREffA = (TH1D*)PROut->Get(Form("hEff1D_PRJpsi_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
  ...
  can->SaveAs(Form("./1DEff_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.pdf",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
  can->SaveAs(Form("./1DEff_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.png",_ymin,_ymax,_ptmin,_ptmax,_centmin,_centmax));
  delete can;
  ...
  for (int i=0; i<nbinsy-1; i++) {
    ...
    string className = "NPJpsi";
    TH1D *hNPGen = (TH1D*)NPOut->Get(Form("hGen_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
    TH1D *hNPRec = (TH1D*)NPOut->Get(Form("hRec_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
    ...
    canv->SaveAs(Form("./1DHist_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.pdf",ymin,ymax,ptmin,ptmax,centmin,centmax));
    canv->SaveAs(Form("./1DHist_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.png",ymin,ymax,ptmin,ptmax,centmin,centmax));
    delete canv;
    ...
    className = "NPJpsi";
    TH1D *hNPEff = (TH1D*)NPOut->Get(Form("hEff_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
    className = "PRJpsi";
    TH1D *hPREff = (TH1D*)PROut->Get(Form("hEff_%s_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d",className.c_str(),ymin,ymax,ptmin,ptmax,centmin,centmax));
    ...
    canv->SaveAs(Form("./1DEff_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.pdf",ymin,ymax,ptmin,ptmax,centmin,centmax));
    canv->SaveAs(Form("./1DEff_Rap%.1f-%.1f_Pt%.1f-%.1f_Cent%d-%d.png",ymin,ymax,ptmin,ptmax,centmin,centmax));
    delete canv;
...
//void lJpsiEff_draw(bool absRapidity=true) {
  if (argc != 4) {
...
//  lJpsiEff_resolAll(absRapidity, logy);
