...
//  chNP->Draw(Form("Centrality>>centNPRec_%s",name.c_str()),recoCut.c_str(),"pe");
  ...
  if (setLogy) {
    ...
    ptPRGen->GetMinimum()==0 ? yaxisMin=1E-6 : yaxisMin = ptPRGen->GetMinimum()*0.5;
  } else {
  ...
  if (setLogy) {
    ...
    ptNPGen->GetMinimum()==0 ? yaxisMin=1E-6 : yaxisMin = ptNPGen->GetMinimum()*0.5;
  } else {
...
int testpt() {
  ...
  double PRWeights[nPRFiles] = {1.0/117512.0, 0.96075/124802.0, 0.2136/172176.0, 0.04385/167336.0, 0.0112585/143944.0, 0.00579/166960.0};
  double NPWeights[nNPFiles] = {1.0/171458.0, 2.25*0.623/172208.0, 2.55*0.245/171178.0, 2.45*0.080/172208.0, 2.0*0.0299/170670.0, 3.35*0.0109/169302.0};
  ...
  double cutAllYPt[] = {100, 0, 2.4, 0, 30};
  double cutMidYPt[] = {100, 0, 1.2, 6.5, 30};
  double cutInterYPt[] = {100, 1.2, 1.6, 6.5, 30};
  double cutForwYPt[] = {100, 1.6, 2.4, 3, 30};
