...
//  const double _ctauforwarray[] = {0, 0.3, 0.6, 0.8, 1.0, 1.5, 2.0, 3.0};
  ...
  if (isPbPb) {
    dirPath = "/home/mihee/cms/RegIt_JpsiRaa/Efficiency/PbPb";
    if (absRapidity)
    ...
    dirPath = "/home/mihee/cms/RegIt_JpsiRaa/Efficiency/pp/RegionsDividedInEta";
    if (absRapidity)
    ...
    for (unsigned int b=0; b<nPtArr; b++) {
      for (unsigned int c=0; c<nCentArr; c++) {
        ...
        if (isPbPb) {
          ...
          feffSimUnf[nidx]->SetParameters(-3.0,2,3);
...
//            feffSimUnf_LowPt[nidx]->FixParameter(3,0.10);
          ...
          feffSimUnf_LowPt[nidx]->SetParameters(-3.0,2,3);
        }
...
//          double truelxy = lxyTrueReco_pfy[nidx]->GetBinContent(bin1);
        ...
        if (isPbPb) {
          ...
          feffProf[nidx]->SetParameters(-3.0,2,3);
        }
...
//          double truelxy = lxyTrueReco_pfy_LowPt[nidx]->GetBinContent(bin1);
        ...
        if (isPbPb) {
          ...
          feffProf_LowPt[nidx]->SetParameters(-3.0,2,3);
        }
